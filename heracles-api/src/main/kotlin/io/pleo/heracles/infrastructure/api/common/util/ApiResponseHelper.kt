...
object ApiResponseHelper {
    ...
    fun createRejectedHeader(webRequest: WebRequest, errorCode: String, errorMessage: String): Header {
        return createBasicHeader(
                requestHeaderMessageId = webRequest.getHeader(Headers.MESSAGE_ID.value),
                requestHeaderGroupId = webRequest.getHeader(Headers.GROUP_ID.value),
    ...
    fun <T> createResponseEntity(responseHeader: Header, body: T): ResponseEntity<T> {
        ...
        headers.add(Headers.GROUP_ID.value, responseHeader.groupId)
        headers.add(Headers.MESSAGE_ID.value, responseHeader.messageId)
        headers.add(Headers.TIMESTAMP.value, responseHeader.timestamp.toString())
    ...
    fun createBasicHeaderFromHttpRequestHeader(httpRequest: HttpServletRequest): Header {
        return Header(
                messageId = httpRequest.getHeader(Headers.MESSAGE_ID.value),
                timestamp = OffsetDateTime.parse(
                        httpRequest.getHeader(Headers.TIMESTAMP.value)
                ),
                groupId = httpRequest.getHeader(Headers.GROUP_ID.value)
        )
    }
